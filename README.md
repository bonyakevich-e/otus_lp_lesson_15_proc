...
4. `TIME`: Как долго процессор был занят процессом. Берём из _/proc/PID/stat_. В этом файле за процессорное время отвечают два значения:
   ...
   ```
6. `COMMAND`: Берём из _/proc/PID/cmdline_. Если процесс запущен без аргументов, то помещаем его в квадратные скобки (так делает `ps`). Если _/proc/PID/cmdline_ пустой - используем _/proc/PID/stat_, второе значение
